...
public class RecomendacionController {
    ...
    @Operation(summary = "Listar recomendaciones por lista de cultivoIds (agroIds)")
    @GetMapping("/mis-cultivos")
    public ResponseEntity<ApiResponse<Page<RecomendacionResponse>>> listarPorMisCultivos(
            @RequestParam List<Long> cultivoIds,
            @PageableDefault(size = 20) Pageable pageable) {
        return ResponseEntity.ok(ApiResponse.ok(
                recomendacionService.listarPorCultivoIds(cultivoIds, pageable)));
    }
